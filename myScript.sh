...
import { ${SINGULAR^}Resolver } from "../../services/http/${SECTION}/${SECTION}.resolver";
import { FormComponent } from "./pages/form/form.component";
import { ListComponent } from './pages/list/list.component';
import { campsTemplate, campsValidation } from "./templates/form.template";
import { template } from "./templates/list.temp.functions";
...
export class FormComponent implements OnInit {
  ...
  activoInactivoOptions :any[]=[ {value:0,description:'Inactivo'},{value:1,description:'Activo'}]
    ...
    <div class="row" >
      ...
      <div class="col-xl-6 pb-2">
        <div class="d-flex align-items-center justify-content-end">
          ...
          <button
            class="btn btn-block btn-primary pl-1 pr-1 font-weight-bold ms-3"
            routerLink="/${SECTION}/crear"
            >
            <i class="fa fa-plus"></i><strong> CREAR</strong>
          </button>
            <button class="btn btn-block btn-secondary ms-3">
              <i class="icon-menu"></i>
            </button>
      </div>
      </div>
...
import { setData, setValueQueryPlus } from '../../templates/list.temp.functions';
...
@ViewChild('topScrollAnchor')topScroll:ElementRef
  ...
  ngOnInit() {    
    ...
    this.activatedRoute.queryParams.subscribe((query) => {
      this.queryParams.queryParams.page=query.page
      this.queryParams.queryParams.take=query.take
      this.queryParams.queryParams.search=query.search
      this.rootService
        .index('${SECTION}',query.page, query.take, query.search)
        .subscribe((res: ArrayResponse<${SINGULAR^}>) => {
            this.listForTemplate = []
            this.${SECTION} = res.data
            this.${SECTION}.forEach(${SINGULAR}=>{
                const ${SINGULAR}Data = setData(${SINGULAR})
                const ${SINGULAR}Query = setValueQueryPlus(${SINGULAR})
  ...
  changeTotalItems(total: number) {
    ...
    this.router.navigate(['/'+ '${SECTION}'+ '/'+'listar'], this.queryParams);
  }
  ...
  templ:MainListTemplate = {} as MainListTemplate
  ${SECTION}: ${SINGULAR^}[] = []
  listForTemplate : ListTemplate[] =[]
  queryPlusSet=[]
}
EOF
cd ..
touch ./templates/list.temp.functions.ts
touch ./templates/form.template.ts
cat>./templates/list.temp.functions.ts<<EOF
import { formatDate } from "../../../shared/functions/formatDate";
import { ${SINGULAR^} } from "../../../shared/models/${SINGULAR}.model";
import { fontName, fontNumber, fontPrimary } from "../../../shared/models/basic/fonts.template.const";
import { MainListTemplate, ListTemplate} from "../../../shared/models/basic/template.list.model";

export const template:MainListTemplate ={
    headIcon: 'fa fa-building mr-4',
    // relations:[
    //     {
...
export function setData(obj:${SINGULAR^}):ListTemplate{
    return {} as ListTemplate
      //MODIFICAR ACORDE A LAS PROPIEDADES DE SU OBJETO
      //header: 'texto  DEL ACORDERON',
      //properties: [
      //{
...
//  ],
 // id: obj.id,
  //SI Alguna Relacion Es unica (utilizar una propiedad en el array de arriba con actions)
  //directoresMedicos: obj.director_medico_id,          Estos ids son para la busqueda de la relacion a la hora de abrir el acordeon
